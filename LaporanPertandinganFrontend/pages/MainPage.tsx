...
const BABAK_LABEL: Record<Babak, string> = {
...
const PEMENANG_LABEL: Record<Pemenang, string> = {
  TIM_A: "Tim A",
  TIM_B: "Tim B",
  DRAW:  "Seri",
};

const BABAK_COLOR: Record<Babak, string> = {
...
const PEMENANG_COLOR: Record<Pemenang, string> = {
...
export default function MainPage() {
  ...
  const [exportTarget, setExportTarget]           = useState<LaporanDetail | null>(null); // null = batch
  ...
  // ── Delete ─────────────────────────────────────────────
  const handleDelete = async (laporan: LaporanDetail) => {
    const label = `${laporan.nama_cabor} — ${laporan.nama_nomor} (${BABAK_LABEL[laporan.babak]})`;
    if (!confirm(`Hapus laporan "${label}"? Data atlet ikut terhapus dan tidak dapat dikembalikan.`)) return;
    try {
      await laporanPertandinganService.delete(laporan.id);
      setList(prev => prev.filter(l => l.id !== laporan.id));
    } catch (e: any) {
  ...
  // ── Quick export satu tanpa tanda tangan ──────────────
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {(filterTanggal || filterBabak || filterPemenang || filterCaborId !== "") && (
            <button
              type="button"
              onClick={() => { setFilterTanggal(""); setFilterBabak(""); setFilterPemenang(""); setFilterCaborId(""); }}
                    ...
                    {list.map((l, i) => (
                      <tr key={l.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition-colors">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-gray-600 dark:text-gray-300">
                          <div>{fmtDate(l.tanggal_pertandingan)}</div>
                          <div className="text-xs text-gray-400">{fmtTime(l.waktu_pertandingan)} WIB</div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${BABAK_COLOR[l.babak]}`}>
                            {BABAK_LABEL[l.babak]}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${PEMENANG_COLOR[l.pemenang]}`}>
                            {PEMENANG_LABEL[l.pemenang]}
                          </span>
                          {l.juara_ke && (
